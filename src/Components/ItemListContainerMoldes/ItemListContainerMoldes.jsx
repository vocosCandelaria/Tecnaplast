...
import "./styleItemListContainerMoldes.css";

const ItemListContainerMoldes = () => {
  ...
  const resultSearch = products.filter((s) => {
    ...
    if (
      s.title.toString().toLowerCase().includes(searchLowerCase) ||
  ...
  return (
    <>
      {CategoryId ? (
        <h2 className="category_title">{CategoryId}</h2>
      ) : (
        <h2 className="category_title">nuestros moldes</h2>
      )}
...
export default ItemListContainerMoldes;
